...
import { d as db, u as users, l as landlordProfiles, t as tenantProfiles, s as staffProfiles } from './db-B3IBLKz5.js';
...
var POST = async ({ request, cookies }) => {
	try {
		...
		if (action === "register") return json({ error: "Tài khoản chủ trọ chỉ được tạo bởi SuperAdmin Roomio." }, { status: 403 });
			...
			const user = await db.query.users.findFirst({ where: eq(users.email, requiredEmail(demoEmail)) });
			if (!user || user.role !== "LANDLORD" || !user.isActive) return json({ error: DEMO_LOGIN_NOT_READY_MESSAGE }, { status: 404 });
			const landlordProfile = await db.query.landlordProfiles.findFirst({ where: eq(landlordProfiles.userId, user.id) });
			if (!landlordProfile) return json({ error: DEMO_LOGIN_NOT_READY_MESSAGE }, { status: 404 });
...
export { POST };
//# sourceMappingURL=_server.ts-BmbM77Ff.js.map
